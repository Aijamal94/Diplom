...
public class BaseClass {
    ...
    public void authorizationUserOrNot() {
        // Запускаем активити перед взаимодействием с UI
        ActivityScenario.launch(AppActivity.class);

        Allure.step("Шаг 1: Проверяем, авторизован пользователь или нет");
        try {
            onView(isRoot()).perform(waitDisplayed(mainPageID, 7000));
            onView(withId(mainPageID)).check(matches(isDisplayed()));

        } catch (Exception e) {
            Allure.step("Открылась страница авторизации, выполняем вход");
